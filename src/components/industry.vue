<template>
<v-container>
    <v-card-text align="center"><font size="4"><b>應用產業</b></font></v-card-text>
    <v-layout row>
            ...
            <v-card color="white">
             ...
             <v-text-field v-model="industry[0].title" label="標題" class="input-group--focused"></v-text-field>
                </v-flex>
                    <v-flex xs6 offset-xs1>
             <v-select attach :items="items" label="類別" offset-y light v-model="industry[0].type"></v-select>
             </v-flex>
            <v-flex xs6 offset-xs1>
             <p>內文<upload-button title="檔案上傳" name="file" accept=".pdf" :selectedCallback="Upload"></upload-button></p>
             <p v-if="industry[0]['file']!==null">{{industry[0]['file'].name}}</p>
                </v-flex>
             <br>
             <v-flex offset-xs3>
...
data () {
    ...
    methods:{
        create(){
          this.industry[0].title='';
          this.industry[0].type='';
          this.industry[0].id='';
          this.industry[0].file=null;
          console.log(this.industry[0])
        },
        createConfirm(){
          let formdata = new FormData(); 
          formdata.append('title', this.industry[0].title);
          formdata.append('file', this.industry[0].file);
          formdata.append('type',this.industry[0].type);
        ...
        update(){
            ...
            this.industry[0].title=this.selected[0].title,
            this.industry[0].type=this.selected[0].type,
            this.industry[0].id=this.selected[0].id
        },
        updateConfirm(){
          console.log(this.industry[0])
          let formdata = new FormData(); 
