<template>
<v-container>
    <v-card-text align="center"><font size="4"><b>衍生品牌</b></font></v-card-text>
    <v-layout row>
            ...
            <v-card color="white">
             ...
             <v-text-field v-model="brand[0].title" label="品牌名稱" class="input-group--focused"></v-text-field>
                </v-flex>
                <v-flex xs6 offset-xs1>
             <p>內文<upload-button title="檔案上傳" name="file" accept=".pdf" :selectedCallback="Upload"></upload-button></p>
             <p v-if="brand[0]['file']!==null">{{brand[0]['file'].name}}</p>
                </v-flex>
             <br>
             <v-flex offset-xs3>
...
data () {
      return {
        ...
        headers: [
          ...
          { text: '品牌名稱', value: 'title',sortable: false, },
   ...
   methods:{
        ...
        createConfirm(){
          let formdata = new FormData();
           formdata.append('title', this.brand[0].title);
            formdata.append('file', this.brand[0].file);
            formdata.append('classID',10);
            console.log(this.brand[0].file);
        ...
        update(){
          ...
          for(var i in this.brand[0]){
            ...
            this.brand[0].title=this.selected[0].title;
            this.brand[0].id=this.selected[0].id;
            console.log(this.brand[0]);
        },
        updateConfirm(){
           let formdata = new FormData();
           formdata.append('title', this.brand[0].title);
            ...
            formdata.append('classID',10);
            let token = localStorage.getItem('token')
            api.articleUpdate(token, this.brand[0].id , formdata).then(res=>{
                alert('修改成功')
                window.location.reload();
            }).catch(error=>{
                alert(error);
